defmodule MyappWeb.EventLogControllerTest do
  use MyappWeb.ConnCase

  alias Myapp.EventLogs

  @create_attrs %{nDateTime: 42, nEventIdn: 42, nEventLogIdn: 42, nIsLog: 42, nReaderIdn: 42, nUserID: 42}
  ...
  def fixture(:event_log) do
    {:ok, event_log} = EventLogs.create_event_log(@create_attrs)
    event_log
  end

  describe "index" do
    test "lists all eventlogs", %{conn: conn} do
      conn = get(conn, Routes.event_log_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing Eventlogs"
    end
  end
